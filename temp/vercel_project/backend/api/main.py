...
from backend.database.src.schemas import (
    UserCreate,
    AccountCreate,
    PositionCreate,
    JobCreate, JobUpdate,
...
@app.post("/cron/research")
async def cron_research():
    """
    Trigger the Researcher workflow.
    Intended to be called by Vercel Cron (every 2 hours).
    """
    try:
        from backend.researcher.server import research as researcher_research, ResearchRequest, DEFAULT_RESEARCH_PROMPT
        req = ResearchRequest(prompt=DEFAULT_RESEARCH_PROMPT)
        result = await researcher_research(req)
        return {"ok": True, "result": result}
    except Exception as e:
        logger.exception("Cron research failed")
        raise HTTPException(status_code=500, detail=str(e))
