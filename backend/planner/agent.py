...
async def invoke_lambda_agent(agent_name: str, function_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Invoke an agent locally (Vercel-friendly).

    Kept the same function name to minimize diffs; it no longer calls AWS Lambda.
    """
    if MOCK_LAMBDAS:
        logger.info(f"[MOCK] Would run {agent_name} with payload: {json.dumps(payload)[:200]}")
        return {"success": True, "message": f"[Mock] {agent_name} completed", "mock": True}

    try:
        job_id = payload.get("job_id") or payload.get("jobId") or ""
        portfolio_data = payload.get("portfolio_data") or payload.get("portfolioData") or {}

        # Run the correct agent handler
        import asyncio

        if agent_name.lower() == "charter":
            from backend.charter.lambda_handler import run_charter_agent as fn
        elif agent_name.lower() == "tagger":
            from backend.tagger.lambda_handler import run_tagger_agent as fn
        elif agent_name.lower() == "reporter":
            from backend.reporter.lambda_handler import run_reporter_agent as fn
        elif agent_name.lower() == "retirement":
            from backend.retirement.lambda_handler import run_retirement_agent as fn
        else:
            raise ValueError(f"Unknown agent_name={agent_name}")

        result = asyncio.run(fn(job_id=job_id, portfolio_data=portfolio_data, db=None))
        # Normalize return shape
        if isinstance(result, dict):
            return result
        return {"success": True, "result": result}
    except Exception as e:
        logger.exception(f"Error running {agent_name} locally")
