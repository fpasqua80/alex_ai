#!/usr/bin/env python3
"""
Seed instrument data for Alex Financial Planner (Postgres / Vercel Postgres).

Replaces the old AWS RDS Data API based seeder.
Uses DATABASE_URL.
...
def verify_allocations():
    """Verify that allocation percentages sum to ~1.0 for each instrument."""
    for inst in INSTRUMENTS:
        alloc = inst.get("allocation_data") or {}
        total = 0.0
        for _, v in alloc.items():
            try:
                ...
                pass
        if alloc and not (0.98 <= total <= 1.02):
            raise ValueError(f"Allocation for {inst['symbol']} sums to {total}, expected ~1.0")
